...
import org.prauga.messages.interactor.MarkAllSeen
import org.prauga.messages.interactor.MarkArchived
import org.prauga.messages.interactor.MarkPinned
import org.prauga.messages.interactor.MarkRead
import org.prauga.messages.interactor.MarkUnarchived
import org.prauga.messages.interactor.MarkUnpinned
import org.prauga.messages.interactor.MarkUnread
import org.prauga.messages.interactor.MigratePreferences
...
class MainViewModel @Inject constructor(
    ...
    override fun bindView(view: MainView) {
        ...
        view.queryChangedIntent
            ...
            .observeOn(AndroidSchedulers.mainThread())
            .withLatestFrom(state) { query, state -> Pair(query, state) }
                .switchMap { (query, currentState) ->
                    if (query.isEmpty()) {
                        if (currentState.page is Searching) {
                            newState {
                                when (activeChip) {
                                    MessageCategory.ARCHIVED -> copy(
                                        page = Archived(
                                            data = archivedData(
                                                currentFilter
                                            )
